...
/**
 * Converts an unsorted list of {@link ServiceExtension} instances into a directed graph based on dependency direction
 */
public class DependencyGraph {
    ...
    private Set<String> getProvidedFeatures(ServiceExtension ext) {
        ...
        allProvides.addAll(scanProviders(ext).nonDefaultProviders().stream().map(ProviderMethod::getReturnType).map(Class::getName).collect(Collectors.toSet()));
        return allProvides;
    }

    private Set<String> getDefaultProvidedFeatures(ServiceExtension ext) {
        return scanProviders(ext).defaultProviders().stream()
